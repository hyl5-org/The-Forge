...
void gles_createShaderReflection(Shader* pProgram, ShaderReflection* pOutReflection, const BinaryShaderDesc* pDesc)
{
    ...
    if (reflection.mVertexInputsCount > 0)
    {
        ...
        for (GLint i = 0; i < reflection.mVertexInputsCount; ++i)
        {
            ...
            GLenum attrType = 0;
            CHECK_GLRESULT(glGetActiveAttrib(glProgram, i, activeAttributeMaxLength, (GLsizei*)&vertexInput.name_size,
                                             (GLint*)&vertexInput.size, &attrType, attrName));
            LOGF(LogLevel::eDEBUG, "Program %u, Attribute %d, size %d, type %u, name \"%s\"", glProgram, i, vertexInput.size, attrType,
                 attrName);
    ...
    if (nUniforms > 0)
    {
        ...
        for (GLint i = 0; i < nUniforms; ++i)
        {
            ...
            LOGF(LogLevel::eDEBUG, "Program %u, Uniform %d, location: %d, size %d, type %u, name \"%s\"", glProgram, i, location, size,
